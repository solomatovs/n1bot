...
class AsyncToolCall:
    """Открытый вызов для потребителя из event loop'а.

    Читатель кадров живёт в отдельном потоке: кадры едут в очередь loop'а
    через call_soon_threadsafe, ошибка чтения приезжает тем же путём и
    поднимается у потребителя.
    """
