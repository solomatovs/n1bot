...
from boba.schema.coercion import MISSING, MinValue
...
def test_required_and_default_params():
    ...
    assert {f.name for f in parsed.schema.fields} == {"query", "limit"}
    assert _field(parsed.schema, "limit").coercer.apply(MISSING) == 10
