"""Гранты субъекта: один SQL, по которому хост и инструменты узнают, какие
строки connections выданы пользователю лично или любой его роли.

Текст запроса живёт в домене, чтобы брокер (выбор строки под вызов) и
плагин connection_list/connection_search (каталог для модели) считали
выдачу и дубли имён одинаково: имя, выданное дважды внутри одного вида, —
дубль, вызов по нему неоднозначен. Домен psycopg не знает: текст несёт
плейсхолдеры идентификаторов `{...}` и параметров `%(...)s`, идентификаторы
подставляет исполнитель по картам ConnectionNames.
