...
class ConfigPath:
    ...
    def parse(cls, source: str) -> Self:
        """
        Парсит путь указанных в dot нотакции: a.b[0].c
            a - это словарь элементов где есть элемент b (массив)
            b[0] - это нулевой элемент массива, который тоже является словарем
            c - элемент словаря b[0]
        """
        body = source
        if not body:
            return cls.root()
