...
def test_full_replacement_string_value():
    ...
    assert bundle.flat.lookup(
        ConfigPath.parse("tool.search.url")
...
def test_reference_preserves_non_string_value_type():
            ...
            DictSource(
                {
                    ...
                    ConfigPath.parse("tool.search.timeout"): StringValue(
                        "@{base.timeout}"
                    ),
                }
            )
    ...
    assert bundle.flat.lookup(
        ConfigPath.parse("tool.search.timeout")
    ).value() == IntValue(30)
            ...
            DictSource(
                {
                    ...
                    ConfigPath.parse("tool.url"): StringValue(
                        "https://@{base.host}/api"
                    ),
                }
            )
...
def test_depth_limit_raises():
    ...
    n = 5
    values: dict[ConfigPath, ScalarValue] = {
