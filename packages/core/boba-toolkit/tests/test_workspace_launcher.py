...
from boba.toolkit.workspace.chain import build_chain_argv
...
class TestFuseMounter:
    ...
    def test_missing_fuse2fs_raises(
        ...
        monkeypatch.setattr(
            "boba.toolkit.workspace.launcher.shutil.which", lambda _: None
        )
    ...
    def test_shutdown_kills_stubborn_daemon(self) -> None:
        ...
        daemon = subprocess.Popen(  # noqa: S603 — фиксированный argv теста
...
class TestCapabilityDropper:
    def test_drop_all_clears_caps(self) -> None:
        code = (
            "from boba.toolkit.workspace.launcher import CapabilityDropper\n"
            "CapabilityDropper().drop_all()\n"
            "print(open('/proc/self/status').read())\n"
        )
        env = {**os.environ, "PYTHONPATH": os.pathsep.join(sys.path)}
        out = subprocess.run(  # noqa: S603 — фиксированный argv теста
