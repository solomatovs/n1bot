"""Групповые лимиты: cgroup v2 leaf на запуск.

Интеграционные тесты используют делегированную зону systemd-сессии
пользователя (user@<uid>.service): создание leaf'а и запись лимитов
разрешены оттуда любому процессу пользователя, а вот миграция ребёнка —
только когда сам pytest запущен внутри этой зоны (launch-конфиг
«Chainlit2: pytest current file» оборачивает python в systemd-run).
"""
