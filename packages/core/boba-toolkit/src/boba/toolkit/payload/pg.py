"""Соединение и приведение строк для payload'ов, работающих с postgres.

Пула здесь нет: каждый вызов payload'а — свой процесс, а значит своё
соединение; цена — рукопожатие (и kerberos) на каждый запрос. Учётные данные
приезжают через stdin, поэтому не видны ни в argv, ни в /proc, ни в логах.
"""
