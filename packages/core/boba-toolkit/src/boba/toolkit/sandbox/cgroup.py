...
Rlimit ограничивает каждый процесс отдельно, cgroup — всё дерево запуска
суммарно. База (cgroup_base) должна быть делегирована процессу приложения:
на хосте это запуск внутри systemd-scope пользователя (см. systemd-run
--user --scope), в docker — собственный cgroup namespace контейнера.
Миграция ребёнка в leaf разрешена ядром, только когда сам процесс
приложения находится под общим делегированным предком с базой.
"""
...
from boba.toolkit.sandbox.profile import SandboxProfile
...
class CgroupManager:
    ...
    def probe_profiles(cls, profiles: Mapping[str, SandboxProfile]) -> None:
        """Доказывает при старте, что запрошенные групповые лимиты применимы.

        Любой отказ — ошибка с именем профиля и параметра конфига: либо
        чинится делегирование, либо параметр убирается из конфига.
        """
