...
Ошибки:
UnknownHostError — for_url: хост URL вне покрытия соединения.
"""
...
class HttpConnection(ConnectionBase):
    ...
    def _address_names_a_host(self) -> HttpConnection:
        ...
        try:
            ...
            msg = f"web connection: address parts do not form a URL: {exc}"
            raise ValueError(msg) from exc

        if not root.host:
            msg = "web connection: address needs a host, set host or netloc"
            raise ValueError(msg)
    ...
    def bound_to(self, host: str) -> HttpConnection:
        """Соединение с конкретным хостом вместо шаблона; порт сохраняется."""
        root = self.root_url()
    ...
    def for_url(self, url: str) -> HttpConnection:
        """Соединение под этот URL: хост обязан попадать под host соединения
        (точный или шаблон `*.domain`); чужой хост — отказ."""
        host = HostPattern.host_of(url)

        if not self.covers(host):
            msg = (
                f"web: host {host!r} is outside the chosen connection "
                f"(it covers {self.host!r}). URL={url!r}"
            )
            raise UnknownHostError(msg)

        return self.bound_to(host)
    ...
    def service_name(self) -> str:
        ...
        if HostPattern(value=host).wildcard:
            msg = f"web connection: SPN needs a concrete host, got pattern {host!r}"
            raise ValueError(msg)
