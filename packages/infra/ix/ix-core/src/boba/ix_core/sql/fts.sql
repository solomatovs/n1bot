/*
Полнотекст по таблицам индекса своего вида: вместо {index} подставляется их объединение
из реестра {schema}.index_table, поэтому запрос не знает ни происхождений, ни поверхностей.
