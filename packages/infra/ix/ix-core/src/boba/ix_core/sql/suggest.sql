/*
Подсказки при наборе по таблицам триграмм: вместо {index} подставляется их объединение
из реестра. Сначала совпадение по префиксу идентификатора через btree
...
имена схлопнуты сразу по всем таблицам, число объектов отдаётся колонкой objects;
node_id это объект лучшего попадания.
