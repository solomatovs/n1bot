...
исполнения нет. Ошибки: SandboxPayloadError — исполнение сорвалось либо
контракт каналов нарушен; PayloadFailureError — стадия сообщила об ожидаемом
отказе конвертом; исключения sink'ов фасада (коллекторов) проходят наверх
как есть — их контракт задаёт потребитель потока.
"""
