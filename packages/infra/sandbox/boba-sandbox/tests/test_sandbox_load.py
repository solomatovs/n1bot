...
from boba.sandbox.profile import BindSpec
...
from boba.toolkit.channels import Channel, ChannelSink
from boba.toolkit.launcher import StageRun, WorkflowStageError
...
class BashPayloadMounts:
    """Код обвязки bash и её зависимости внутри песочницы: бинды и env."""

    REPO: ClassVar[Path] = Path(__file__).resolve().parents[4]

    TOOLKIT: ClassVar[str] = "/opt/toolkit"
    SHELL: ClassVar[str] = "/opt/shell"
    SITE: ClassVar[str] = "/opt/site"

    PATH: ClassVar[str] = "/usr/local/bin:/usr/bin:/bin"

    @classmethod
    def sources(cls) -> dict[str, str]:
        toolkit = cls.REPO / "packages" / "core" / "boba-toolkit" / "src"
        shell = cls.REPO / "packages" / "tools" / "boba-tool-shell" / "src"
        site = Path(pydantic.__file__).resolve().parents[1]

        return {
            str(toolkit): cls.TOOLKIT,
            str(shell): cls.SHELL,
            str(site): cls.SITE,
        }
...
class AllowAllNodes:
    """Права вне сессии: в реестре стенда только его собственный узел bash."""
...
class _BrokenSink(ChannelSink):
    """Потребитель продукта, который срывается на первом же куске байт."""
...
class CallFacts(BaseModel):
    """Процессные факты вызова плюс головы его каналов.

    Форма повторяет прежний RunResult: нагрузочные проверки смотрят на код
    возврата и вывод, а не на устройство каналов.
    """
...
class LoadCall(BaseModel):
    """Исход одного вызова стенда: то же поле result, что у фасадов."""
...
class LoadCaller:
    """Вызов bash-узла графом из одной стадии: команда, код возврата, вывод."""

    HEAD_BYTES: ClassVar[int] = 1 << 20

    def __init__(self, profile: SandboxProfile, user_id: str, thread_id: str) -> None:
        ready = BashPayloadMounts.applied(profile)

        defs: dict[str, StageDef] = {}
        for name, node in BashStage.stages().items():
            defs[name] = StageDef.of(node, ready)
        ...
        self._run = StageRun(self._caller)
    ...
    def call_text(
        ...
        """Одна команда в песочнице; sink — приёмник её stdout по мере работы."""
        literal: str | None = None
        if stdin:
            literal = stdin

        with CallStand.bound():
            ...
            stage = outcome.outcome_of(BashStage.NAME)
            stdout = self._head(outcome, Channel.TOOL_PAYLOAD)
            stderr = self._head(outcome, Channel.TOOL_STDERR)
        ...
        return LoadCall(result=facts)

    def _head(self, outcome: WorkflowOutcome, channel: Channel) -> str:
        head = self._run.head(outcome, BashStage.NAME, channel, self.HEAD_BYTES)
...
class LoadStand:
    ...
    def caller(
        ...
        **overrides: object,
        ...
        return LoadCaller(self.profile(**overrides), user_id, thread_id)
...
class TestParallelLoad:
    ...
    @staticmethod
    def _write_and_read(stand: LoadStand, user_id: str, index: int) -> LoadCall:
...
class CallReport(BaseModel):
    ...
    @classmethod
    def of(cls, outcome: LoadCall) -> CallReport:
        return cls(
...
class TestAbnormalTermination:
    ...
    @staticmethod
    def _report(caller: LoadCaller, command: str) -> CallReport:
    ...
    def test_failing_output_consumer_frees_image(self, stand: LoadStand) -> None:
        ...
        with pytest.raises(RuntimeError, match="consumer is broken"):
            stand.caller("sink").call_text(
                "echo noise; sleep 300", stdin="", sink=_BrokenSink()
            )
...
from test_sandbox_load import LoadCaller
...
outcome = LoadCaller(profile, user_id, "t1").call_text(command, stdin="")
