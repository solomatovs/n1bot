...
class TestPlainTextReader:
    async def test_utf8_one_section_with_doc_type(self) -> None:
        ...
        if len(secs) != 1:
            raise AssertionError("len(secs) == 1")
        if secs[0].order != 0:
            raise AssertionError("secs[0].order == 0")
        if secs[0].content != "# Заголовок\n\nТекст.":
            raise AssertionError('secs[0].content == "# Заголовок\\n\\nТекст."')
        if secs[0].metadata.get(ReaderKeys.DOC_TYPE) != "md":
            raise AssertionError('secs[0].metadata.get(ReaderKeys.DOC_TYPE) == "md"')
    ...
    async def test_falls_through_to_next_encoding(self) -> None:
        ...
        if secs[0].content != "Привет":
            raise AssertionError('secs[0].content == "Привет"')
...
class TestTextMedia:
    def test_routes_cover_text_media_types(self) -> None:
        ...
        if set(routes) != set(TextMedia.DOC_TYPE_BY_MEDIA_TYPE):
            raise AssertionError("set(routes) == set(TextMedia.DOC_TYPE_BY_MEDIA_TYPE)")
