...
Декоратор `@tool` — для функций; для классов используется свободная фабрика
`tool_factory(obj)`. Имя по умолчанию — `type(obj).__name__`, через `name=`
переопределяется. Если класс — dataclass, тот же тип можно скормить
`schema_from_dataclass` для получения config-schema.
"""
...
from boba.coercion import MinValue
