...
Ошибки:
ClickHouseError — до базы не достучаться.
PayloadError видов FailureKind (sql_failed — СУБД отклонила запрос, no_input —
    вставке не подали поток); все объявлены ожидаемыми и едут пользователю
    текстом.
"""
