...
class PostgresFtsPluginConfig:
    ...
    dsn: Annotated[
        str,
        "libpq DSN; read-only/statement_timeout задаются параметрами в самом DSN.",
        ParseString(),
    ]
    indexes: list[IndexSpec]
    min_pool_size: Annotated[
        int,
        "Минимальный размер pool'а.",
        ParseInt(),
        MinValue(1),
    ] = 1
    ...
    connect_timeout_sec: Annotated[
        float,
        "Таймаут получения connection из pool'а.",
        ParseFloat(),
    ] = 10.0
    snippet_options: Annotated[
        str,
    ...
    max_top_k: Annotated[
        int,
        "Жёсткий потолок параметра top_k для fts_search.",
        ParseInt(),
        MinValue(1),
    ] = 20
