...
Применение:
    BOBA_CONFIG_PATH=./local/config.toml \\
        .venv/bin/python -m boba.tool.kb.cli.confluence.ingest.folder
"""
...
__all__ = ["ConfluenceIngestFolderCliConfig", "main"]

logger = logging.getLogger("boba.tool.kb.cli.confluence.ingest.folder")

_PIPELINE_ID: PipelineId = PipelineId("kb.confluence.ingest.folder")
_FS_DISPATCH_READER_ID: ReaderId = ReaderId("ext.confluence_folder_dispatch")


class ConfluenceIngestFolderCliConfig(BobaFlatSettings):
...
def main() -> int:
    ...
    cfg = ConfluenceIngestFolderCliConfig()  # pyright: ignore[reportCallIssue]
    ...
    try:
        ...
        logger.exception("confluence.ingest.folder FAILED")
        return 1
...
def _run_ingest(cfg: ConfluenceIngestFolderCliConfig) -> dict[str, Any]:
