...
class ConfluenceJson:
    ...
    def as_dict(v: Any) -> dict[str, Any]:
        """v, если это dict, иначе пустой dict."""
        return v if isinstance(v, dict) else {}

    @staticmethod
    def as_int(v: Any, *, default: int) -> int:
        """int(v) или default для None/нечислового."""
        if v is None:
            return default
        try:
            return int(v)
    ...
    def title(data: dict[str, Any]) -> str:
        """data.title или пустая строка."""
        return str(data.get("title") or "")

    @staticmethod
    def version_number(data: dict[str, Any]) -> int | None:
        """version.number как int; None если отсутствует/нечисловой."""
    ...
    def source_url(data: dict[str, Any]) -> str:
        """Каноничный URL объекта: _links.base + _links.webui; "" если нет обоих."""
    ...
    def response_base(data: dict[str, Any]) -> str:
        """Response-level _links.base (без trailing /); "" если нет.

        Search отдаёт base на уровне ответа, а webui — в каждом хите; для
        склейки кликабельного URL хита нужен именно этот base.
        """
    ...
    def results(data: dict[str, Any]) -> list[dict[str, Any]]:
        """results[] из top-level или вложенного page.results; [] если нет."""
    ...
    def next_link(data: dict[str, Any]) -> str | None:
        """_links.next (next-page path) или None."""
...
class ConfluenceJsonDecoder(Decoder):
    ...
    def decode(self, value: RawDocument) -> RawDocument:
        ...
        # Каноничный кликабельный URL страницы — от самого Confluence:
        # _links.base + _links.webui (без fallback на хардкод-конструкцию).
