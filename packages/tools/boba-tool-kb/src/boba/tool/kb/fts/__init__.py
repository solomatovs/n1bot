...
- `fts_search(query, top_k)` — websearch_to_tsquery по таблице из конфига.
  Секция `[tool.kb.search.fts]`. Таблица фиксируется
  `FtsSearchConfig.whitelist` (`IndexSpec`): один whitelist-индекс на tool.
  LLM таблицу не выбирает.

Каждый tool self-contained: connection + whitelist + snippet_options + max_top_k
