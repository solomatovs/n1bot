...
Поверх generic `boba-html-parser`: переопределяет извлечение текста и
...
# Служебный браузерный anchor
# который Confluence ставит в начало страницы при экспорте
# и который не имеет смысла в нашем случае
# т.к. не ссылается ни на что конкретное
# а просто служит точкой входа для браузера
# Игнорируем его при извлечении anchor'ов из heading'ов
# чтобы не плодить бесполезные anchor'ы вида
# "scroll-bookmark-1", "scroll-bookmark-2" и т.п.
_GOBACK = "_GoBack"
