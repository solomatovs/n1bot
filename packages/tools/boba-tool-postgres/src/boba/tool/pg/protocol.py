"""Контракт postgres-payload'а: SQL исполняется в песочнице.

Соединение описывается libpq-параметрами: приложение отдаёт их вместе с
готовым SQL, обратно получает строки. Секреты едут через stdin.
"""
