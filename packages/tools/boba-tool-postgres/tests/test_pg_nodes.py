"""Узлы postgres: обогащение args, вырожденный граф фасада, форматы выгрузки."""
...
class TestPgEnrichment:
    ...
    def test_copy_args_carry_the_format(self) -> None:
        ...
        assert request.copy_format is PgCopyFormat.CSV
        assert PgCopyNode.out_of()(request) is StreamFormat.CSV

    def test_default_copy_format_is_text(self) -> None:
        enrich = PgCopyNode.enricher(pg_config())

        request = PgCopyRequest.model_validate(
            enrich({"connection_name": "main", "sql": "select 1"})
        )

        assert request.copy_format is PgCopyFormat.TEXT
        assert PgCopyNode.out_of()(request) is StreamFormat.TEXT
        assert "FORMAT TEXT, HEADER" in request.copy_format.statement(request.sql)
...
class TestPgCaller:
    ...
    def test_copy_spec_names_the_copy_node(self) -> None:
        ...
        assert spec.nodes[0].args["copy_format"] == PgCopyFormat.CSV
...
class TestPgFacades:
    ...
    def test_export_over_max_bytes_becomes_error_result(self) -> None:
        oversized = b"x" * 2000
        launcher = _StageLauncher(oversized, {"rows": 1})
        built = build_pg_tools(pg_config(), lambda tool: launcher)
        tool = next(t for t in built if t.name == "pg_export")
