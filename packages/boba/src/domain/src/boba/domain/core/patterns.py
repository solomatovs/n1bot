...
class ConverterInputError(ConverterError):
    ...
    Этот класс остаётся «плоским» (только message) — структурный
    контекст (FieldSpec, ConfigKey) живёт в подклассах верхних слоёв
    (declaration.FieldConverterError, наполняемом в ConfigSection.build),
    чтобы patterns.py не зависел от типов вышестоящих уровней.
    """


class MissingValueError(ConverterInputError):
    """Маркер: значение не было предоставлено (MISSING / null).

    Сигнализирует «никакой источник не отдал значение» (вместо «значение
    отдано, но невалидно»). Используется Required-конвертером и
    переносится валидатором в подкласс с контекстом (FieldMissingError),
    позволяя верхнему уровню отличать «нечего парсить» от «парсилось,
    но не сошлось».
    """
