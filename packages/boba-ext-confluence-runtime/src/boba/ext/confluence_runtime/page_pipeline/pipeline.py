...
from boba.confluence import (
    anchor_for,
    collect_headings,
    parse_html,
)
from boba.ext.confluence_runtime.page_pipeline.errors import (
    ConfluencePageHttpError,
    ConfluencePageNotFoundError,
    ConfluencePageResponseError,
)
from boba.ext.confluence_runtime.page_pipeline.models import (
...
class ConfluencePagePipeline:
    ...
    def fetch(self, page_id: str) -> ConfluencePageContent:
        ...
        try:
            resp = self._client.get(
                f"{self._CONTENT_PATH}/{page_id}", params=params,
            )
    ...
    def _extract_headings(html: str) -> list[ConfluencePageHeading]:
        ...
        return [
            ConfluencePageHeading(
                level=h.level, text=h.text, anchor=anchor_for(h),
            )
