...
from boba.chainlit2.agent.tools.kb.embedding import LocalFastEmbedEmbedderFactory
from boba.chainlit2.agent.tools.kb.kb import PostgresKnowledgeBaseConfig
from boba.chainlit2.agent.tools.kb.migrations import Migrations
from boba.chainlit2.agent.tools.kb.postgres import KbPool
...
class KbSchema:
    ...
    def setup(self) -> None:
        ...
        dim = LocalFastEmbedEmbedderFactory.build(self._cfg.embedding).dim()
        with pool.connection() as conn:
            Migrations.ensure_vector_index(
                conn, dim=dim, schema_cfg=self._cfg.tables
            )
