...
Модели описывают обе стороны, но исполняется payload внутри песочницы, где
пакетов boba нет, — там те же поля собираются на голом json. Расхождение
ловится валидацией ответа по схеме на стороне приложения.
"""
...
from boba.chainlit2.agent.tools.liteparse.protocol import ParseParams
...
class DocPathRequest(DocRequest):
    """Запрос без дополнительных полей: read_document и document_outline."""

    READ: ClassVar[str] = "read_document"
