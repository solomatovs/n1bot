...
class SecretCipher:
    """Шифрует все SecretStr модели на любой глубине, остальное оставляет как есть.

    Обход идёт по значениям, а не по аннотациям: вложенные модели, варианты
    discriminated union, списки и словари покрываются без разбора типов.
    """
