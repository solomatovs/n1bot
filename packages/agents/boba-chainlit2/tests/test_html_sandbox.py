...
_PAYLOAD_MAIN = Path(__file__).resolve().parents[1] / "payloads" / "parse" / "main.py"
...
def _config() -> SandboxEntryConfig:
    return SandboxEntryConfig.model_validate(
        {
            "profile": _PROFILE,
            "override": {},
...
def caller(monkeypatch: pytest.MonkeyPatch) -> HtmlCaller:
    from boba.chainlit2.agent.tools.html import caller as caller_module
