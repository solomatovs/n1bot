...
from boba.hermes.chat.auth.ldap import (
    ADDirectory,
    ADUserEntry,
...
from boba.hermes.errors import (
    AuthenticationError,
    AuthorizationError,
    BaseError,
...
class KerberosAuth:
    ...
    def _init_mapping(self):
        self._principal_roles: StaticRolesProvider | None = None
        self._principal_roles_ex: StaticExcludeProvider | None = None
        ...
        if roles := self._config.roles:
            if roles.principal:
                self._principal_roles = StaticRolesProvider(roles.principal)

            if roles.principal_ex:
                self._principal_roles_ex = StaticExcludeProvider(roles.principal_ex)
