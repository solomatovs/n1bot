...
from boba.hermes.chat.auth.kerberos import KerberosAuthConfig
from boba.hermes.chat.auth.ldap import LdapAuthConfig
from boba.hermes.chat.auth.local import LocalAuthConfig
...
class HermesConfig(BaseModel):
    """Подключение к hermes api_server и провижининг профилей."""

    model_config = ConfigDict(extra="ignore")

    base_url: str = Field(
        description="Адрес api_server, напр. http://hermes:8642",
    )
    api_key: str = Field(
        description="API_SERVER_KEY; уходит в заголовок Authorization: Bearer.",
    )
    ...
    read_timeout: float = Field(
        default=600.0,
        description="Ход агента идёт одним SSE-стримом, поэтому таймаут длинный.",
    )
...
AuthConfig = Annotated[
    KerberosAuthConfig | LdapAuthConfig | LocalAuthConfig,
...
class AppConfig(BaseModel):
    ...
    hermes: Annotated[
        HermesConfig,
        Field(description="Бэкенд агента: api_server и профили."),
    ]
