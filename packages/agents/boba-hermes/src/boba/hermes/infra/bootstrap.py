...
from boba.hermes.chat.auth import PasswordAuthCallbackInstaller
from boba.hermes.errors import DomainErrorMiddleware
from boba.hermes.infra import providers
...
from boba.hermes.infra.di import Container
...
def _use_chainlit_middleware(app: FastAPI, config: ChainlitExtendConfig):
    ...
    import boba.hermes.chat.callback  # type: ignore # noqa: F401, PLC0415
...
def _use_auth(config: AppConfig, container: Container) -> None:
    ...
    for auth in config.auth:
        if isinstance(auth, KerberosAuthConfig):
            from boba.hermes.chat.auth.kerberos import (  # noqa: PLC0415
                KerberosAuth,
            )
            ...
            from boba.hermes.chat.auth.local import (  # noqa: PLC0415
                LocalAuth,
            )
            ...
            from boba.hermes.chat.auth.ldap import (  # noqa: PLC0415
                LdapAuth,
            )
