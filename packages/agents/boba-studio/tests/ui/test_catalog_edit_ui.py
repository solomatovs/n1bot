...
from playwright.sync_api import Browser, FloatRect, Locator, Page, expect
...
def _prompt_name(page: Page, mark: str, name: str) -> None:
    dialog = Canvas.dialog(page, mark)
...
class TestNodePanel:
    ...
    def test_moving_a_card_does_not_reload_its_panel(
        ...
        expect(panel.get_by_test_id("node-card")).to_be_visible(
            timeout=Canvas.LIVE_TIMEOUT_MS
        )
...
class TestLive:
    ...
    def test_publish_conflict_offers_rebase_then_publishes(
        ...
        expect(rebase).to_have_text(
            f"update to v{version}", timeout=Canvas.LIVE_TIMEOUT_MS
        )

        page.get_by_test_id("publish-button").click()
        conflict = Canvas.dialog(page, "publish-conflict")
...
class TestStaleness:
    def test_new_snapshot_version_marks_nodes_stale_and_upgrade_is_blocked(
        self,
        page: Page,
        stand: StandProcess,
        catalog_api: Api,
        ...
        устаревшим по событию, панель называет причину, в списке черновик
        помечен «behind»; «upgrade» останавливается с полосой проблем и чипом
        «needs attention», привязки прежние; перенацеливание узла на живой
        объект снимает устаревание, и upgrade переводит черновик."""
        _open_draft(page, stand, draft_id)
        catalog = page.get_by_test_id("catalog-page")
        expect(catalog).to_have_attribute("data-stale", "0")
        expect(page.get_by_test_id("upgrade-button")).to_have_count(0)
        ...
        expect(stale).to_have_attribute(
            "data-stale", "true", timeout=Canvas.LIVE_TIMEOUT_MS
        )
        expect(catalog).to_have_attribute("data-stale", "1")
        expect(page.get_by_test_id("stale-chip")).to_have_text("1 stale")
        row = page.get_by_test_id("processes-list").locator("li[data-draft]").first
        expect(row).to_have_attribute("data-behind", "true")
        expect(row.get_by_test_id("behind-chip")).to_have_text("behind")
        ...
        # upgrade — задача с той же полосой, что синхронизация: ход, итог
        page.get_by_test_id("upgrade-button").click()
        progress = page.get_by_test_id("upgrade-progress")
        expect(progress).to_have_attribute(
            "data-status", "done", timeout=Canvas.LIVE_TIMEOUT_MS
        )
        expect(progress).to_contain_text("upgrade of the draft")
        expect(progress).to_contain_text("0 moved, 1 blocked")
        blocked = page.locator('[data-notice="upgrade-blocked"]')
        expect(blocked).to_be_visible(timeout=Canvas.LIVE_TIMEOUT_MS)
        expect(blocked.get_by_test_id("upgrade-problems")).to_have_text("1 problem(s)")
        expect(row.get_by_test_id("attention-chip")).to_have_text("needs attention · 1")
        assert catalog_api.state(draft_id)["draft"]["pins"] == {
            catalog_seed.connection_id: 1
        }
        ...
        expect(progress).to_contain_text(
            "1 moved, 0 blocked", timeout=Canvas.LIVE_TIMEOUT_MS
        )
        expect(page.get_by_test_id("upgrade-button")).to_have_count(
            0, timeout=Canvas.LIVE_TIMEOUT_MS
        )
        expect(blocked).to_have_count(0)
        # перенацеливание увело панель на вкладку подключений
        page.get_by_role("tab", name="process").click()
        expect(row).to_have_attribute("data-behind", "false")
        expect(row.get_by_test_id("attention-chip")).to_have_count(0)
        assert catalog_api.state(draft_id)["draft"]["pins"] == {
            catalog_seed.connection_id: 2
        }

    def test_upgrade_all_from_the_entry_marks_the_blocked_process(
        self,
        page: Page,
        stand: StandProcess,
        catalog_api: Api,
        ...
        """Новая версия снимка без ed_returns: на входе процесс помечен
        «behind», «upgrade all» у списка переводит совместимые и оставляет
        этот с чипом «needs attention»; привязка процесса прежняя."""
        tables = [name for name in catalog_seed.tables if name != Ed.RETURNS]
        catalog_seed.next_version([*tables, Ed.EVENTS, Ed.ARCHIVE])
        before = next(
            item
            for item in catalog_api.processes()
            if item["name"] == catalog_seed.process_name
        )

        CatalogPage.HOME.open(page, stand)
        row = page.get_by_test_id("processes-list").locator(
            f'li[data-process="{catalog_seed.process_name}"]'
        )
        expect(row).to_have_attribute("data-behind", "true")
        expect(row.get_by_test_id("behind-chip")).to_have_text("behind")
        upgrade_all = page.get_by_test_id("upgrade-all")
        expect(upgrade_all).to_have_accessible_name(
            re.compile(r"^upgrade all processes")
        )
        ...
        expect(progress).to_contain_text("upgrade of all lagging processes")
        expect(progress).to_contain_text("0 moved, 1 blocked")
        expect(row.get_by_test_id("attention-chip")).to_have_text(
            "needs attention · 1", timeout=Canvas.LIVE_TIMEOUT_MS
        )
        expect(row).to_have_attribute("data-attention", "1")
        process = next(
            item
            for item in catalog_api.processes()
            if item["name"] == catalog_seed.process_name
        )
        # blocked: версия процесса и привязка прежние
        assert process["latest_version"] == before["latest_version"]
        assert process["pins"] == before["pins"]
        assert process["behind"] is True
