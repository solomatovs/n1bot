...
@pytest.fixture
def process_url(stand: StandProcess, catalog_seed: Seed) -> str:
    """Адрес опубликованной страницы процесса сида."""
    return CatalogPage.PROCESS.url(stand, process_id=catalog_seed.process_id)
...
class TestToolbar:
    def test_zoom_buttons_and_fit_view_change_the_viewport_scale(
        self, tabs: Tabs, stand: StandProcess, process_url: str
    ) -> None:
        page = tabs.page("admin")
        _open(page, process_url)
    ...
    def test_tidy_up_lays_the_nodes_out_again(
        self, tabs: Tabs, stand: StandProcess, catalog_seed: Seed, process_url: str
    ) -> None:
...
class TestDialogClosing:
    def test_name_prompt_closes_by_cross_escape_and_cancel_without_changes(
        ...
        page = tabs.page("admin")
        _open(page, CatalogPage.DRAFT.url(stand, draft_id=draft_id))
        open_prompt = page.get_by_test_id("group-button")
        prompt = Canvas.dialog(page, "group-name")
    ...
    def test_process_and_share_dialogs_cancel_and_close_without_changes(
        ...
        shares = Canvas.dialog(page, "share")
    ...
    def test_keep_the_draft_as_is_leaves_a_stale_draft_untouched(
        ...
        expect(page.get_by_test_id("rebase-button")).to_have_text(
            f"update to v{other}", timeout=Canvas.LIVE_TIMEOUT_MS
        )
...
class TestDiscard:
    def test_discard_asks_first_then_closes_the_draft_and_leaves_the_page(
        ...
        dialog = Canvas.dialog(page, "draft-discard")
...
class TestShare:
    def test_share_link_opens_the_process_for_a_guest_read_only(
        ...
        _open(page, process_url)
        page.get_by_test_id("share-button").click()
        dialog = Canvas.dialog(page, "share")
        ...
        expect(row).to_have_count(0, timeout=Canvas.LIVE_TIMEOUT_MS)
        CatalogPage.SHARED.open(guest, stand, token=token)
...
class TestToasts:
    def test_rejected_operation_shows_an_error_toast_that_dismisses_on_click(
        ...
        toast.click()
        expect(toast).to_have_count(0, timeout=Canvas.LIVE_TIMEOUT_MS)
...
class TestEntryNavigation:
    def test_first_edit_of_a_published_process_starts_a_draft(
        ...
        try:
            ...
            prompt = Canvas.dialog(page, "draft-name")
...
class TestHomeButtons:
    ...
    def test_plus_in_the_processes_group_starts_a_draft_of_a_new_process(
        self, tabs: Tabs, stand: StandProcess, catalog_api: Api, process_url: str
    ) -> None:
...
class TestAnonymousAndNarrow:
    def test_anonymous_tab_is_sent_to_the_sign_in_page(
        self, tabs: Tabs, stand: StandProcess, process_url: str
    ) -> None:
        """Без входа страница каталога, как и остальные страницы studio,
        уводит на вход; холст не рисуется."""
        page = tabs.page("")
        page.goto(process_url)
        page.wait_for_url(re.compile(r"/workflow/login$"), timeout=30_000)
        expect(page.locator(Selector.NODE)).to_have_count(0)
    ...
    def test_narrow_screen_keeps_dialogs_and_editing_within_the_viewport(
        ...
        dialog = Canvas.dialog(page, "group-name")
        expect(dialog).to_be_visible()
        assert Css.box(dialog.get_by_role("dialog")).right <= Viewport.NARROW["width"]
        assert no_horizontal_scroll(page)
