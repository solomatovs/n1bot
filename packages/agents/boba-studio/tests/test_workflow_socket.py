...
@pytest.fixture
def service(
    store: WorkflowStore, studio_config: StudioRuntimeConfig, bus: MemoryMessageBus
) -> WorkflowService:
    probe = Probe()
...
async def test_run_events_reach_a_namespace_on_another_instance(  # noqa: PLR0913
    ...
    cfg = studio_config.data_layer.postgres.model_copy(update={"dbname": test_database})
    cluster = studio_config.cluster
    schema = studio_config.data_layer.db_schema
...
async def test_user_events_reach_the_room_over_postgres(  # noqa: PLR0913
    ...
    bus = PgMessageBus(
        cfg,
        studio_config.data_layer.db_schema,
        "node1-studio",
        AppName.STUDIO,
