"""Инструменты над каталогом данных в процессе studio: зовут CatalogService
от имени субъекта вызова (REST, workflow).
...
черновик относительно его базовой версии; catalog_open отдаёт ссылку на
страницу процесса или черновика; catalog_sync снимает структуру
подключения инструментом его вида и ждёт итога.

Ошибки: ErrorResult — нет контекста вызова, нет прав, процесс или черновик не
...
class CatalogToolError(StrEnum):
    ...
    UPGRADE_REFUSED = "catalog_upgrade_refused"


class CatalogLinkKind(StrEnum):
    """Что открывает ссылка каталога."""
...
class CatalogPrompt(StrEnum):
    ...
    UPGRADE_TARGET = (
        "What to upgrade to the latest connection snapshots: a process name or "
        "id (uuid), a draft id (uuid), or empty string for every published "
        "process that lags behind. A process is moved when its nodes and flows "
        "still hold on the new snapshots (objects present, no column dropped, "
        "column types unchanged or widened); otherwise it stays as is and the "
        "answer lists the problems: node or flow, connection, column, type "
        "change. Fix them in a draft (retarget the node, drop the pair) and "
        "upgrade again."
    )
...
class CatalogTools:
    """Тела инструментов каталога: субъект из контекста вызова, ответы моделям."""
    ...
    async def sync(self, connection: str, schemas: str) -> tuple[str, ToolResult]:
        ...
        try:
            ...
            caller = SyncCaller.of_context(context)
            scope = SyncScope(schemas=NameList.parse(schemas))
    ...
    async def upgrade(self, target: str) -> tuple[str, ToolResult]:
        ...
        try:
            subject = CallContext.current_subject()
            service = await self._service()
            kind, entity_id = await self._upgrade_target(service, subject, target)
            started = await service.start_upgrade(
                subject, kind, entity_id, AuthorVia.LLM
            )
    ...
    ) -> tuple[UpgradeTarget, UUID | None]:
        """Цель по имени процесса, id процесса либо id черновика; пусто — все."""
        stripped = raw.strip()
        if stripped == "":
            return UpgradeTarget.ALL, None

        try:
            process = await self._process_by(service, subject, stripped)
        except ProcessNotFoundError:
            return UpgradeTarget.DRAFT, self._uuid(stripped)

        return UpgradeTarget.PROCESS, process.id
...
def build_catalog_tools(
    ...
    async def catalog_upgrade(
        ...
        a new process version with the new pins. Blocked: the pins stay and
        the result lists what breaks (removed objects, dropped columns,
        narrowed column types) so the problems can be fixed in a draft. Sync
        does not do this by itself."""
        return await tools.upgrade(target)
