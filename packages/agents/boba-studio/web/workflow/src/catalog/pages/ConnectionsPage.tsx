...
import { connectionRows, type Access, type ConnectionRow, type SyncedConnection } from "../model/catalog";
...
export function ConnectionsPage(): ReactElement {
  ...
  const [state, reload] = useLoadable(
    useCallback(() => load(api, workflowApi), [api, workflowApi]),
    { keep: true },
  );
  ...
  const renderLists = ({ access, rows, foreign, doc }: Lists): ReactElement => {
          ...
          >
            ...
            <Toolbar>
              <Button
                tone="danger"
                onClick={() => {
                  run(workflowApi.removeConnection(dialog.row.id), "connection deleted");
                }}
  ...
  return (
    <Async
      state={state}
      fill
      title={{ loading: "loading", failed: "connections are not available" }}
      render={renderLists}
    />
  );
}
...
function ConnectionListRow({ row, canEdit, probe, onOpen, onCheck, onSync, onDelete }: RowProps): ReactElement {
  ...
  return (
    <ListRow data-connection={view.name} data-synced={synced !== undefined}>
      <ListName to={PageUrls.catalog.connection(view.id)} title={view.available ? undefined : "the connection type is not installed"}>
        {view.name}
      </ListName>
      <ListAside>
...
async function load(api: CatalogApi, workflowApi: WorkflowApi): Promise<Lists> {
  ...
  const doc = new SchemaDoc(parseSchema(await workflowApi.connectionSchema()));
