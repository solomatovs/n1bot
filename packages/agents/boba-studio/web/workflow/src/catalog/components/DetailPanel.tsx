...
import { useObjectCard, type SharedCardSource } from "../hooks/useObjectCard";
import {
  flowLabel,
  pairsLabel,
  renderRef,
...
function SourceCard({ api, catalog, node, cardSource, onOpenObject }: CardProps): ReactElement {
  ...
  let shared: SharedCardSource | undefined = undefined;
  if (cardSource.kind === "shared") {
    shared = { token: cardSource.token, nodeId: node.id };
  }
  const state = useObjectCard(api, node.ref.connection_id, version, node.ref, shared);
