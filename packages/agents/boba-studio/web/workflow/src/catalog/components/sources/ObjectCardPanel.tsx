...
export function ObjectCardPanel({ card, flat = false, onOpenObject }: Props): ReactElement {
  return (
    <div data-testid="object-card" data-card={card.card} data-path={card.ref.path.join("/")}>
      <Panel page={!flat} flat={flat}>
        {card.card === "pg_relation" && <PgRelationView card={card} onOpenObject={onOpenObject} />}
        {card.card === "pg_routine" && <PgRoutineView card={card} />}
        {card.card === "pg_sequence" && <PgSequenceView card={card} />}
        {card.card === "pg_type" && <PgTypeView card={card} />}
        {card.card === "ch_table" && <ChTableView card={card} />}
        {card.card === "ch_dictionary" && <ChDictionaryView card={card} />}
      </Panel>
    </div>
  );
}
...
function CardHead({ eyebrow, name, kind, comment }: Head): ReactElement {
  return (
    <PanelHead
      eyebrow={eyebrow}
      name={name}
      mono
      actions={<Chip tone="muted">{kind}</Chip>}
...
function PgRelationView({ card, onOpenObject }: RelationViewer): ReactElement {
...
function PgRoutineView({ card }: Viewer<"pg_routine">): ReactElement {
...
function PgSequenceView({ card }: Viewer<"pg_sequence">): ReactElement {
...
function PgTypeView({ card }: Viewer<"pg_type">): ReactElement {
...
function ChTableView({ card }: Viewer<"ch_table">): ReactElement {
...
function ChDictionaryView({ card }: Viewer<"ch_dictionary">): ReactElement {
