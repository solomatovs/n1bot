...
export const StaleReasonSchema = z.enum(["object_removed", "column_removed", "column_changed"]);
...
export const UpgradeStatusSchema = z.enum(["moved", "blocked"]);
export const UpgradeTargetSchema = z.enum(["process", "draft", "all"]);

/** Запуск upgrade — задача, как синхронизация: цель, ход и итог. */
export const UpgradeRunSchema = z.object({
  id: z.string(),
  target: UpgradeTargetSchema,
  process_id: z.string().nullable(),
...
export const UpgradeSchema = z.object({
  ...
  pins_before: PinsSchema,
  pins_after: PinsSchema,
  problems: z.array(StaleSchema),
  version: z.number().nullable(),
  at: z.string(),
});

export const UpgradeReportSchema = z.object({
  run: UpgradeRunSchema,
...
export type UpgradeReport = z.infer<typeof UpgradeReportSchema>;
