...
function Scalar({
  ...
  issues,
  onChange,
}: NodeProps & { node: Node }): ReactElement {
...
function ObjectFields(props: NodeProps & { node?: Extract<Node, { kind: "object" }> }): ReactElement {
  ...
  const rank = (name: string, property: JsonSchema): number => {
    ...
    if ("nullable" in inner && inner.nullable) {
      return 2;
    }

    return 1;
  };
  const ordered = [...node.properties].sort(([nameA, a], [nameB, b]) => rank(nameA, a) - rank(nameB, b));
