...
// boba: id последнего сообщения пользователя по дереву сообщений, без обхода DOM
const lastUserMessageIdOf = (messages: IStep[]): string | undefined => {
  for (let index = messages.length - 1; index >= 0; index--) {
    const message = messages[index];
    if (message.steps?.length) {
      const nested = lastUserMessageIdOf(message.steps);
      if (nested) return nested;
    }
    if (message.type === 'user_message') return message.id;
  }
  return undefined;
};
...
export default function ScrollContainer({
  ...
  const spacerFrameRef = useRef(0);
  const lastUserMessageId = useMemo(
    () => lastUserMessageIdOf(messages),
    [messages]
  );
  ...
  // boba: пересчёт спейсера читает offsetHeight всех сообщений после последнего
  // вопроса — forced layout; на каждый токен это делалось синхронно. Теперь не
  // чаще кадра отрисовки, а элемент вопроса ищется только при его смене
  const scheduleSpacerUpdate = useCallback(() => {
    if (spacerFrameRef.current) return;
    spacerFrameRef.current = requestAnimationFrame(() => {
      spacerFrameRef.current = 0;
      updateSpacerHeight();
    });
  }, [updateSpacerHeight]);

  useEffect(() => () => cancelAnimationFrame(spacerFrameRef.current), []);
  ...
  useEffect(() => {
    ...
    const inner = document.getElementById(`step-${lastUserMessageId}`);
    const step = inner?.closest('[data-step-type="user_message"]');
    lastUserMessageRef.current = (step as HTMLDivElement | null) ?? null;
  }, [lastUserMessageId]);
