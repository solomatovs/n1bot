...
const addMessageToParent = (
  ...
  // boba: родитель ищется один раз на список, а не для каждого соседа
  const parentInside = hasMessageById(messages, parentId);
  ...
  const nextMessages = messages.map((msg) => {
    ...
    } else if (parentInside && msg.steps) {
...
// boba: пачка токенов одним обновлением дерева: соседние токены одного
// сообщения склеиваются, дерево копируется один раз на сообщение, а не на токен
const applyTokens = (messages: IStep[], tokens: IToken[]): IStep[] => {
  const merged: IToken[] = [];
