...
import { Chip, Note, Panel, PanelHead } from "../../ui";
import "./sources.css";
...
export function DiffPanel({ diff, title }: Props): ReactElement {
  return (
    <div data-testid="source-diff" data-entries={diff.entries.length}>
      <Panel page>
        <PanelHead eyebrow="diff" name={title} actions={<Chip tone="muted">{diff.entries.length} changes</Chip>} />
        {diff.entries.length === 0 && <Note mark="diff-empty">nothing changed</Note>}
        <ul className="diff">
