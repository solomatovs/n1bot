 ...
 * добавленных и удалённых групп; на черновике — карандаш и корзина. */
export function GroupFrame({ data }: NodeProps<GroupNode>): ReactElement {
  const status = data.showDiff ? data.status : "unchanged";

  return (
    <div className="group-frame" data-status={status} data-testid="group-frame" data-group={data.group.name}>
