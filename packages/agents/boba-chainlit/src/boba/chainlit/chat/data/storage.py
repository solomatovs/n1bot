...
Ошибки:
StorageError — операция не выполнена.
StorageFullError — в образе нет места.
StorageNotFoundError — объекта нет в хранилище. Ничего сверх этого списка
    наружу не выходит: системные и чужие исключения упаковывает StorageGuard,
    через который проходит каждая операция StorageClient. Клиенту достаточно
    ловить StorageError — подклассы разбираются, когда отказ надо показать по-
    разному (404 против сбоя, нет места против прочего).
"""
