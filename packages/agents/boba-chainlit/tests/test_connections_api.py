...
def _profile_name(app_config: AppConfig) -> str:
...
def _query(app_config: AppConfig, **extra: str) -> dict[str, str]:
    query = {"profile": _profile_name(app_config)}
    query.update(extra)
    return query
...
async def test_lists_granted_rows_with_masked_secrets(
    ...
    reply = await client.get(
        ...
        params={"profile": _profile_name(app_config)},
    )
    ...
    for secret in _secrets_of(app_config.data_layer.postgres):
...
async def test_kind_filter_narrows_the_list(
    ...
    reply = await client.get(
        f"{ApiVersion.V1}{ConnectionUrl.CONNECTIONS}",
        params={"profile": _profile_name(app_config), "kind": "web"},
    )

    assert reply.status_code == 200, reply.text
    assert [row["id"] for row in reply.json()] == [granted["web"]]
...
async def test_owner_creates_replaces_and_deletes_own_connection(
    ...
    listed = await client.get(
        f"{ApiVersion.V1}{ConnectionUrl.CONNECTIONS}", params=_query(app_config)
    )
    ...
    deleted = await client.delete(
        f"{ApiVersion.V1}/connections/{row['id']}", params=_query(app_config)
    )
...
async def test_shared_and_foreign_rows_are_not_editable(
    ...
    shared_delete = await client.delete(
        f"{ApiVersion.V1}/connections/{granted['web']}", params=_query(app_config)
    )
    assert shared_delete.status_code == 403

    foreign = await client.delete(
        f"{ApiVersion.V1}/connections/{granted['stranger']}", params=_query(app_config)
    )
