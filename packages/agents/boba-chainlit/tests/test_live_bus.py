...
async def _bus(
    ...
    bus = PgMessageBus(
        cfg, app_config.data_layer.db_schema, name, AppName.STUDIO, app_config.cluster
    )
    bus._pool_ref = pool
    await bus.setup()
    await bus.start()
...
async def buses(
    ...
    first = await _bus(app_config, test_database, pool, "node1-studio")
    second = await _bus(app_config, test_database, pool, "node2-studio")
    try:
        yield first, second
    finally:
        await first.stop()
...
async def test_failing_subscriber_stops_the_listener_and_the_bus_refuses(
    ...
    bus = await _bus(app_config, test_database, pool, "node3-studio")
    try:
        scope = Scope.chat(str(uuid4()))
        seen: list[int] = []
