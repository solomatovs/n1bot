...
Стенд KDC тот же, что у входа chainlit: токен браузера — из test_sso_login_flow.
"""
...
def chainlit_context() -> None:
    """Обмен идёт через studio: сессия chainlit не нужна."""
...
class Stand:
    """Studio с SSO стенда: секрет — из конфига, как у chainlit."""
...
@pytest.fixture
def stand(raw_config: Any, app_config: AppConfig) -> Stand:
    return Stand(raw_config, app_config)
...
async def test_sign_in_sets_a_cookie_with_the_ticket_and_returns_to_next(
    ...
    reply = await client.get(
        ...
        headers=_negotiate(Browser.token(tmp_path)),
    )
...
async def test_refresh_issues_a_fresh_ticket_for_the_session(
    ...
    signed_in = await client.get(
        f"{ApiVersion.V1}{SignInUrl.SSO}", headers=_negotiate(Browser.token(tmp_path))
    )
    ...
    reply = await client.post(
        ...
        headers={
            **_negotiate(Browser.token(tmp_path)),
