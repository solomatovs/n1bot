...
Затем сам CLI:
```bash
pip install -e ./packages/boba-cli
```

После этого `boba-cli` работает из терминала без `PYTHONPATH`.

## Запуск

Один прогон:
```bash
boba-cli --model qwen2.5-14b --query "Прочитай файл X и расскажи о чём он"
```

REPL (интерактивный режим, история диалога сохраняется в памяти):
```bash
boba-cli --model qwen2.5-14b
