...
def split_sections(
    ...
    """Разбить markdown на Section'ы по heading'ам.

    Section.body — строки **после** heading-строки до следующего heading'а
    (включая пустые), без trailing whitespace. Если до первого heading'а
    есть текст (preamble), он становится Section с heading=None.
    ...
    if not headings:
        body = text.strip()
        if body:
            sections.append(Section(heading=None, body=body))
        return sections
    ...
    for i, h in enumerate(headings):
        ...
        sections.append(Section(heading=h, body=body))
    return sections
