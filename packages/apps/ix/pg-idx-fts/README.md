...
База задаётся подсекцией `[ix.idx_fts.postgres]` — профилем boba-db-postgres: host, dbname,
...
`[ix.idx_fts.krb]` даёт krb5.conf и каталог кэшей билетов для kerberos-профиля. Соединения
