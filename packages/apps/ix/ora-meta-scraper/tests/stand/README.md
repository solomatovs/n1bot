...
- `ddl/`: демонстрационный набор в схеме `EDGE_DEMO`: таблицы с identity, виртуальной
  ...
  триггеры (в том числе instead of), функция, процедура, пакет, тип. В файле ровно
  один statement, текст уходит драйверу как есть; ворота по версии объявлены у файла
  в `DemoDataset` модуля `ora_scraper_stand.py`.
