...
1. Соединение по профилю, версия словаря из `sys.registry$` (`cid = 'CATALOG'`);
   версия сравнивается с воротами файлов `@min`/`@max` по длине ворот.
2. `scrape/*.sql` в порядке имён. Параметров у файлов нет: границы задаются
   внутри запроса подзапросом по `sys.user$` — снимаются схемы всех пользователей, не
   принадлежащих Oracle (`bitand(user$.spare1, 256) = 0`, так же считает
   `dba_users.oracle_maintained`). Каждый результат целиком уходит в `raw_<name>`.
3. После последнего файла ещё раз все секции `@verify`: ключ строки и `row_version`.
...
-- @min 12.1               версия словаря, с которой вариант применим
-- @max 19                 и по которую; сравнение по длине ворот: 19.3 проходит @max 19
<запрос выборки, последняя колонка row_version>
-- @verify
<запрос сверки: ключ и row_version, те же фильтры>
```
...
Потоковость проверяет `tests/test_streaming_memory.py` (`-m load`): демо-набор и тот же
...
Замер 2026-09-22 на 12.2, 18, 21 и 23: 140 MiB при 310 применённых строках и те же
140 MiB при 106 000. Строки идут из курсора Oracle (`arraysize = 2000`) в `COPY`
по одной, в память они не собираются.
