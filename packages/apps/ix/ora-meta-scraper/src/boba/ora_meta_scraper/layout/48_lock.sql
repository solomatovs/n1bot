-- Сессионный advisory-замок на scope источника: сервис базы. Берётся в autocommit до
-- begin и держится через всю транзакцию 50_apply.sql, снимается 55_unlock.sql после
...
select
    pg_advisory_lock(
        hashtextextended(scheme || '://' || host || ':' || port || '/' || database, 0)
    ) as scope_locked
from
    raw_source;
