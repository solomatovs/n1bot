...
2. `scrape/*.sql` в порядке имён (первая цифра это волна). Для каждого `@name`
   выбирается единственный файл, чьи ворота `@min`/`@max` подходят под версию.
...
-- @params dbs             какие массивы нужны запросу; в тексте они {dbs:Array(String)}
-- @collect dbs name       собрать массив dbs из колонки name результата
-- @min 24.4               версия сервера, с которой вариант применим
-- @max 26.5               и по которую; сравнение по длине ворот: 26.5.3 проходит @max 26.5
<запрос выборки, последняя колонка row_version>
-- @verify
<запрос сверки: ключ и row_version, те же фильтры и параметры>
```
