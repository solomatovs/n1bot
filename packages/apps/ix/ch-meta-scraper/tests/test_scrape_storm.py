"""Шторм: десятки одновременных прогонов по всем целям стенда ClickHouse в одну базу
...
import pytest
from ch_scraper_stand import DemoDataset, Golden, IxSource, IxStand, IxStandDatabase
from pydantic import BaseModel

from boba.ch_meta_scraper.worker import ScrapeWorkerError
from boba.db.postgres import AsyncPostgresPool
from boba.db.postgres.profile import PostgresConfig

pytestmark = [pytest.mark.load, pytest.mark.anyio]

logger = logging.getLogger("ch-storm")
...
class TestScrapeStorm:
    async def test_storm_then_control_pass_reaches_golden(
        ...
        for source in ix_stand.ch_sources:
            if source.demo:
        ...
        async with Killer(ix_stand.ix_profile, ix_stand.database) as killer:
            outcomes = await Storm(ix_database, ix_stand.ch_sources).run()
