...
with gone as (
    ...
    where node_id = %(node_id)s and surface = %(surface)s::{schema}.surface_e and aspect = %(aspect)s::{schema}.pg_idx_aspect_e
      and chunk_no >= %(chunk_count)s
    returning 1
)
insert into {schema}.pg_idx_emb_e5_1024 (node_id, surface, aspect, chunk_no, content, content_hash, emb)
select %(node_id)s, %(surface)s::{schema}.surface_e, %(aspect)s::{schema}.pg_idx_aspect_e, c.chunk_no, c.content, %(content_hash)s, c.emb::halfvec(1024)
