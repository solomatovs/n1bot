...
with rel as (
    select
        ...
        t.partition_bound
    from
        {schema}.pg_meta_table t
        join {schema}.node n on n.id = t.node_id
    union all
    select
        n.id,
        n.surface,
        ...
        null
    from
        {schema}.pg_meta_view v
        join {schema}.node n on n.id = v.node_id
),
cols as (
    select
...
input as (
    select
        r.node_id,
        r.surface,
        initcap(r.kind) || ' ' || r.schema_name || '.' || r.name
            || coalesce(E'\nComment: ' || r.comment, '')
            || case
                when r.row_estimate >= 0
                then E'\nRows (estimate): ' || r.row_estimate::bigint
                else ''
            end
...
from
    todo
where
    pg_try_advisory_lock(hashtextextended('pg_llm_description', node_id))
limit
    %(batch)s;
