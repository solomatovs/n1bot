...
md5 материала, который ушёл в модель, indexer_hash это отпечаток пары «поверхность, аспект»:
модель, бюджет входа, промпт пары из {schema}.surface_prompt, шаблоны свёртки и схема
ответа. Смена любого из них переводит в очередь объекты только этой пары. Внешнего
ключа на {schema}.node нет намеренно. Аспект llm_description объявляется в словаре здесь,
а строки surface_aspect описатель добавляет при старте цикла для каждой поверхности, у
которой есть аспект класса describer_input (run/05_declare.sql).
*/
do $$ begin
