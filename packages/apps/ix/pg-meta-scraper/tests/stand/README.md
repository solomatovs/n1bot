...
`ix-core`, затем `schema/` скрапера; оба накатывает `SchemaUpgrade`. На каждом источнике пересоздаётся база `edge_demo` из `ddl/`.

- `ddl/`: демонстрационный набор, в файле ровно один statement. Ворота по версии и
  вкусу (min/max, only/unless gp) объявлены у файла в `DemoDataset` модуля
  `scraper_stand.py`; варианты одной таблицы различаются буквой в имени
  (identity/serial/greenplum, partition 11/10/inherits и т. д.).
