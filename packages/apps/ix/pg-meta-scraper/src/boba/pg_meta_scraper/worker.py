...
class Pipeline:
    ...
    def run(self) -> Sequence[ApplyRow]:
        with psycopg.connect(self._cfg.source_dsn, autocommit=True, application_name=self.APP_NAME) as src, \
             psycopg.connect(self._cfg.ix_dsn, autocommit=True, application_name=self.APP_NAME) as ix:
