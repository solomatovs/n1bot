...
select 'db', d.oid, 'pg_meta_database', a
...
insert into stage_node (kind, oid, surface, address)
select 'nsp', ns.oid, 'pg_meta_schema', a
...
select 'col', at.attrelid, at.attnum, 'pg_meta_column', a
from raw_attribute at
join stage_node r on r.kind = 'rel' and r.oid = at.attrelid and r.surface in ('pg_meta_table', 'pg_meta_view'),
     lateral (select r.address || jsonb_build_object('column', at.attname)) as x(a);

-- Constraint'ы таблиц (под таблицей) и доменов (под типом)
insert into stage_node (kind, oid, surface, address)
select 'con', con.oid, 'pg_meta_constraint', a
...
insert into stage_node (kind, oid, surface, address)
select 'typ', t.oid, 'pg_meta_type', a
from raw_type t
...
insert into stage_node (kind, oid, surface, address)
select 'proc', p.oid, 'pg_meta_routine', a
from raw_proc p
...
insert into stage_node (kind, oid, surface, address)
select 'trg', tg.oid, 'pg_meta_trigger', a
from raw_trigger tg
...
insert into stage_node (kind, oid, surface, address)
select 'stx', sx.oid, 'pg_meta_statistics', a
