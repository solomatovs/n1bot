...
select n.id
from {schema}.node n, raw_source s
...
select count(*) as locked_node from (
    select n.id from {schema}.node n join scope_node sc on sc.id = n.id order by n.id for update of n
) l;

select count(*) as locked_tree from (
    select t.id from {schema}.tree t join scope_node sc on sc.id = t.node_id order by t.id for update of t
) l;

select count(*) as locked_edge from (
    select e.id from {schema}.edge e
...
select count(*) as locked_pg_meta_edge from (
    select m.edge_id, m.role, m.side, m.ordinal from {schema}.pg_meta_edge m
...
select count(*) as locked_pg_meta_database from (
    select x.node_id from {schema}.pg_meta_database x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_meta_schema from (
    select x.node_id from {schema}.pg_meta_schema x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_meta_table from (
    select x.node_id from {schema}.pg_meta_table x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_meta_column from (
    select x.node_id from {schema}.pg_meta_column x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_meta_view from (
    select x.node_id from {schema}.pg_meta_view x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_meta_index from (
    select x.node_id from {schema}.pg_meta_index x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_meta_sequence from (
    select x.node_id from {schema}.pg_meta_sequence x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_meta_routine from (
    select x.node_id from {schema}.pg_meta_routine x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_meta_constraint from (
    select x.node_id from {schema}.pg_meta_constraint x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_meta_trigger from (
    select x.node_id from {schema}.pg_meta_trigger x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_meta_type from (
    select x.node_id from {schema}.pg_meta_type x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_meta_statistics from (
    select x.node_id from {schema}.pg_meta_statistics x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;


-- 2. node
with cur as (
    select n.id, n.surface, n.address from {schema}.node n join scope_node sc on sc.id = n.id
), del as (
    select c.id from cur c
    left join stage_node s on s.address = c.address
    where s.address is null or s.surface <> c.surface
), done as (
    delete from {schema}.node x using del d where x.id = d.id returning x.id
)
insert into apply_log select 'node_delete', (select count(*) from del), (select count(*) from done);

with cur as (
    select n.surface, n.address from {schema}.node n join scope_node sc on sc.id = n.id
), ins as (
    select s.surface, s.address from stage_node s
    left join cur c on c.address = s.address
    where c.address is null
), done as (
    insert into {schema}.node (surface, address)
...
-- 3. tree
with cur as (
    select t.id, t.node_id, t.parent_id from {schema}.tree t join scope_node sc on sc.id = t.node_id
), want as (
    ...
    delete from {schema}.tree x using del d where x.id = d.id returning x.id
)
...
with cur as (
    ...
    insert into {schema}.tree (node_id, parent_id)
...
-- 4. edge: пары src, tgt
with cur as (
    select e.id, e.node_src_id, e.node_tgt_id from {schema}.edge e join scope_node sc on sc.id = e.node_src_id
), want as (
    ...
    delete from {schema}.edge x using del d where x.id = d.id returning x.id
)
...
with cur as (
    ...
    insert into {schema}.edge (node_src_id, node_tgt_id, surface, weight)
...
with cur as (
    ...
    from {schema}.pg_meta_edge m join {schema}.edge e on e.id = m.edge_id join scope_node sc on sc.id = e.node_src_id
), want as (
    ...
    delete from {schema}.pg_meta_edge x using del d
    where x.edge_id = d.edge_id and x.role = d.role::{schema}.pg_meta_edge_role_e and x.side = d.side and x.ordinal = d.ordinal
    returning x.edge_id
)
...
with cur as (
    ...
    insert into {schema}.pg_meta_edge (edge_id, role, side, ordinal, is_key)
    select edge_id, role::{schema}.pg_meta_edge_role_e, side, ordinal, is_key from ins
...
with cur as (
    select x.node_id, name, owner, encoding, collate_name, ctype, comment from {schema}.pg_meta_database x join scope_node sc on sc.id = x.node_id
), want as (
    ...
    delete from {schema}.pg_meta_database x using del d where x.node_id = d.node_id returning x.node_id
)
...
with cur as (
    ...
    insert into {schema}.pg_meta_database (node_id, name, owner, encoding, collate_name, ctype, comment)
...
with cur as (
    select x.node_id, name, owner, comment from {schema}.pg_meta_schema x join scope_node sc on sc.id = x.node_id
), want as (
    ...
    delete from {schema}.pg_meta_schema x using del d where x.node_id = d.node_id returning x.node_id
)
...
with cur as (
    ...
    insert into {schema}.pg_meta_schema (node_id, name, owner, comment)
...
with cur as (
    select x.node_id, schema_name, name, kind, owner, tablespace, persistence, partition_bound, row_estimate, pages, has_index, has_triggers, distribution, storage, comment from {schema}.pg_meta_table x join scope_node sc on sc.id = x.node_id
), want as (
    ...
    delete from {schema}.pg_meta_table x using del d where x.node_id = d.node_id returning x.node_id
)
...
with cur as (
    ...
    insert into {schema}.pg_meta_table (node_id, schema_name, name, kind, owner, tablespace, persistence, partition_bound, row_estimate, pages, has_index, has_triggers, distribution, storage, comment)
...
with cur as (
    select x.node_id, schema_name, relation_name, relation_kind, name, ordinal, data_type, not_null, default_expr, identity, generated, comment from {schema}.pg_meta_column x join scope_node sc on sc.id = x.node_id
), want as (
    ...
    delete from {schema}.pg_meta_column x using del d where x.node_id = d.node_id returning x.node_id
)
...
with cur as (
    ...
    insert into {schema}.pg_meta_column (node_id, schema_name, relation_name, relation_kind, name, ordinal, data_type, not_null, default_expr, identity, generated, comment)
...
with cur as (
    select x.node_id, schema_name, name, kind, owner, comment from {schema}.pg_meta_view x join scope_node sc on sc.id = x.node_id
), want as (
    ...
    delete from {schema}.pg_meta_view x using del d where x.node_id = d.node_id returning x.node_id
)
...
with cur as (
    ...
    insert into {schema}.pg_meta_view (node_id, schema_name, name, kind, owner, comment)
...
with cur as (
    select x.node_id, schema_name, table_name, name, access_method, is_unique, is_primary, is_exclusion, is_valid, columns, expression, predicate, comment from {schema}.pg_meta_index x join scope_node sc on sc.id = x.node_id
), want as (
    ...
    delete from {schema}.pg_meta_index x using del d where x.node_id = d.node_id returning x.node_id
)
...
with cur as (
    ...
    insert into {schema}.pg_meta_index (node_id, schema_name, table_name, name, access_method, is_unique, is_primary, is_exclusion, is_valid, columns, expression, predicate, comment)
...
with cur as (
    select x.node_id, schema_name, name, owner, data_type, start_value, increment, min_value, max_value, cycle, comment from {schema}.pg_meta_sequence x join scope_node sc on sc.id = x.node_id
), want as (
    ...
    delete from {schema}.pg_meta_sequence x using del d where x.node_id = d.node_id returning x.node_id
)
...
with cur as (
    ...
    insert into {schema}.pg_meta_sequence (node_id, schema_name, name, owner, data_type, start_value, increment, min_value, max_value, cycle, comment)
...
with cur as (
    select x.node_id, schema_name, name, kind, language, identity_args, result_type, volatility, security_definer, owner, comment from {schema}.pg_meta_routine x join scope_node sc on sc.id = x.node_id
), want as (
    ...
    delete from {schema}.pg_meta_routine x using del d where x.node_id = d.node_id returning x.node_id
)
...
with cur as (
    ...
    insert into {schema}.pg_meta_routine (node_id, schema_name, name, kind, language, identity_args, result_type, volatility, security_definer, owner, comment)
...
with cur as (
    select x.node_id, schema_name, table_name, name, kind, definition, is_deferrable, is_deferred, is_validated, on_update, on_delete, match_type, comment from {schema}.pg_meta_constraint x join scope_node sc on sc.id = x.node_id
), want as (
    ...
    delete from {schema}.pg_meta_constraint x using del d where x.node_id = d.node_id returning x.node_id
)
...
with cur as (
    ...
    insert into {schema}.pg_meta_constraint (node_id, schema_name, table_name, name, kind, definition, is_deferrable, is_deferred, is_validated, on_update, on_delete, match_type, comment)
...
with cur as (
    select x.node_id, schema_name, table_name, name, timing, events, row_level, enabled, comment from {schema}.pg_meta_trigger x join scope_node sc on sc.id = x.node_id
), want as (
    ...
    delete from {schema}.pg_meta_trigger x using del d where x.node_id = d.node_id returning x.node_id
)
...
with cur as (
    ...
    insert into {schema}.pg_meta_trigger (node_id, schema_name, table_name, name, timing, events, row_level, enabled, comment)
...
with cur as (
    select x.node_id, schema_name, name, kind, base_type, enum_labels, comment from {schema}.pg_meta_type x join scope_node sc on sc.id = x.node_id
), want as (
    ...
    delete from {schema}.pg_meta_type x using del d where x.node_id = d.node_id returning x.node_id
)
...
with cur as (
    ...
    insert into {schema}.pg_meta_type (node_id, schema_name, name, kind, base_type, enum_labels, comment)
...
with cur as (
    select x.node_id, schema_name, name, table_name, kinds, comment from {schema}.pg_meta_statistics x join scope_node sc on sc.id = x.node_id
), want as (
    ...
    delete from {schema}.pg_meta_statistics x using del d where x.node_id = d.node_id returning x.node_id
)
...
with cur as (
    ...
    insert into {schema}.pg_meta_statistics (node_id, schema_name, name, table_name, kinds, comment)
