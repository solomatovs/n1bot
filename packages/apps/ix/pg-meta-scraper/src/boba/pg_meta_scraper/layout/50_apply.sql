...
select count(*) as locked_pg_meta_edge
from (
    select
        m.edge_id,
        m.role,
...
select count(*) as locked_pg_meta_database
from (
    select
        x.node_id
    from
...
select count(*) as locked_pg_meta_schema
from (
    select
        x.node_id
    from
...
select count(*) as locked_pg_meta_table
from (
    select
        x.node_id
    from
...
select count(*) as locked_pg_meta_column
from (
    select
        x.node_id
    from
...
select count(*) as locked_pg_meta_view
from (
    select
        x.node_id
    from
...
select count(*) as locked_pg_meta_index
from (
    select
        x.node_id
    from
...
select count(*) as locked_pg_meta_sequence
from (
    select
        x.node_id
    from
...
select count(*) as locked_pg_meta_routine
from (
    select
        x.node_id
    from
...
select count(*) as locked_pg_meta_constraint
from (
    select
        x.node_id
    from
...
select count(*) as locked_pg_meta_trigger
from (
    select
        x.node_id
    from
...
select count(*) as locked_pg_meta_type
from (
    select
        x.node_id
    from
...
select count(*) as locked_pg_meta_statistics
from (
    select
        x.node_id
    from
...
del as (
    ...
    delete from {schema}.pg_meta_edge x
    using
        del d
    where
        x.edge_id = d.edge_id
            and x.role = d.role::{schema}.pg_meta_edge_role_e
            and x.side = d.side
...
ins as (
    ...
    insert into {schema}.pg_meta_edge
        (edge_id, role, side, ordinal, is_key)
    select
        edge_id, role::{schema}.pg_meta_edge_role_e, side, ordinal, is_key
    from
        ins
...
del as (
    ...
    from
        cur c
        left join want w on w.node_id = c.node_id
            and row(w.name, w.owner, w.encoding, w.collate_name, w.ctype, w.comment)
                is not distinct from
                row(c.name, c.owner, c.encoding, c.collate_name, c.ctype, c.comment)
    where
        w.node_id is null
),
done as (
    delete from {schema}.pg_meta_database x
    using
        del d
    where
        x.node_id = d.node_id
...
ins as (
    ...
    from
        want w
        left join cur c on c.node_id = w.node_id
            and row(c.name, c.owner, c.encoding, c.collate_name, c.ctype, c.comment)
                is not distinct from
                row(w.name, w.owner, w.encoding, w.collate_name, w.ctype, w.comment)
    where
        c.node_id is null
),
done as (
    insert into {schema}.pg_meta_database
        (node_id, name, owner, encoding, collate_name, ctype, comment)
    select
        node_id, name, owner, encoding, collate_name, ctype, comment
    from
        ins
...
del as (
    ...
    from
        cur c
        left join want w on w.node_id = c.node_id
            and row(w.name, w.owner, w.comment)
                is not distinct from
                row(c.name, c.owner, c.comment)
    where
        w.node_id is null
),
done as (
    delete from {schema}.pg_meta_schema x
    using
        del d
    where
        x.node_id = d.node_id
...
ins as (
    ...
    from
        want w
        left join cur c on c.node_id = w.node_id
            and row(c.name, c.owner, c.comment)
                is not distinct from
                row(w.name, w.owner, w.comment)
    where
        c.node_id is null
),
done as (
    insert into {schema}.pg_meta_schema
        (node_id, name, owner, comment)
    select
        node_id, name, owner, comment
    from
        ins
...
del as (
    ...
    delete from {schema}.pg_meta_table x
    using
        del d
    where
        x.node_id = d.node_id
...
ins as (
    ...
    insert into {schema}.pg_meta_table
        (
            node_id,
            schema_name,
            name,
...
del as (
    ...
    delete from {schema}.pg_meta_column x
    using
        del d
    where
        x.node_id = d.node_id
...
ins as (
    ...
    insert into {schema}.pg_meta_column
        (
            node_id,
            schema_name,
            relation_name,
...
del as (
    ...
    from
        cur c
        left join want w on w.node_id = c.node_id
            and row(w.schema_name, w.name, w.kind, w.owner, w.comment)
                is not distinct from
                row(c.schema_name, c.name, c.kind, c.owner, c.comment)
    where
        w.node_id is null
),
done as (
    delete from {schema}.pg_meta_view x
    using
        del d
    where
        x.node_id = d.node_id
...
ins as (
    ...
    from
        want w
        left join cur c on c.node_id = w.node_id
            and row(c.schema_name, c.name, c.kind, c.owner, c.comment)
                is not distinct from
                row(w.schema_name, w.name, w.kind, w.owner, w.comment)
    where
        c.node_id is null
),
done as (
    insert into {schema}.pg_meta_view
        (node_id, schema_name, name, kind, owner, comment)
    select
        node_id, schema_name, name, kind, owner, comment
    from
        ins
...
del as (
    ...
    delete from {schema}.pg_meta_index x
    using
        del d
    where
        x.node_id = d.node_id
...
ins as (
    ...
    insert into {schema}.pg_meta_index
        (
            node_id,
            schema_name,
            table_name,
...
del as (
    ...
    delete from {schema}.pg_meta_sequence x
    using
        del d
    where
        x.node_id = d.node_id
...
ins as (
    ...
    insert into {schema}.pg_meta_sequence
        (
            node_id,
            schema_name,
            name,
...
del as (
    ...
    delete from {schema}.pg_meta_routine x
    using
        del d
    where
        x.node_id = d.node_id
...
ins as (
    ...
    insert into {schema}.pg_meta_routine
        (
            node_id,
            schema_name,
            name,
...
del as (
    ...
    delete from {schema}.pg_meta_constraint x
    using
        del d
    where
        x.node_id = d.node_id
...
ins as (
    ...
    insert into {schema}.pg_meta_constraint
        (
            node_id,
            schema_name,
            table_name,
...
del as (
    ...
    delete from {schema}.pg_meta_trigger x
    using
        del d
    where
        x.node_id = d.node_id
...
ins as (
    ...
    insert into {schema}.pg_meta_trigger
        (node_id, schema_name, table_name, name, timing, events, row_level, enabled, comment)
    select
        node_id, schema_name, table_name, name, timing, events, row_level, enabled, comment
    from
        ins
...
del as (
    ...
    from
        cur c
        left join want w on w.node_id = c.node_id
            and row(w.schema_name, w.name, w.kind, w.base_type, w.enum_labels, w.comment)
                is not distinct from
                row(c.schema_name, c.name, c.kind, c.base_type, c.enum_labels, c.comment)
    where
        w.node_id is null
),
done as (
    delete from {schema}.pg_meta_type x
    using
        del d
    where
        x.node_id = d.node_id
...
ins as (
    ...
    from
        want w
        left join cur c on c.node_id = w.node_id
            and row(c.schema_name, c.name, c.kind, c.base_type, c.enum_labels, c.comment)
                is not distinct from
                row(w.schema_name, w.name, w.kind, w.base_type, w.enum_labels, w.comment)
    where
        c.node_id is null
),
done as (
    insert into {schema}.pg_meta_type
        (node_id, schema_name, name, kind, base_type, enum_labels, comment)
    select
        node_id, schema_name, name, kind, base_type, enum_labels, comment
    from
        ins
...
del as (
    ...
    from
        cur c
        left join want w on w.node_id = c.node_id
            and row(w.schema_name, w.name, w.table_name, w.kinds, w.comment)
                is not distinct from
                row(c.schema_name, c.name, c.table_name, c.kinds, c.comment)
    where
        w.node_id is null
),
done as (
    delete from {schema}.pg_meta_statistics x
    using
        del d
    where
        x.node_id = d.node_id
...
ins as (
    ...
    from
        want w
        left join cur c on c.node_id = w.node_id
            and row(c.schema_name, c.name, c.table_name, c.kinds, c.comment)
                is not distinct from
                row(w.schema_name, w.name, w.table_name, w.kinds, w.comment)
    where
        c.node_id is null
),
done as (
    insert into {schema}.pg_meta_statistics
        (node_id, schema_name, name, table_name, kinds, comment)
    select
        node_id, schema_name, name, table_name, kinds, comment
    from
        ins
