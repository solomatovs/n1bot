/*
pg-idx-vector, схема: таблица ix.pg_idx_emb_e5_1024 и частичные HNSW-индексы по парам
...
docs/pg-meta-scraper/schema/00_surface.sql, словарь аспектов из схемы любого индексатора
(pg-idx-fts или pg-idx-trgm). Внешнего ключа на ix.node нет намеренно.
*/
create extension if not exists vector;

create table if not exists ix.pg_idx_emb_e5_1024 (
...
create index if not exists pg_idx_emb_e5_1024__pg_meta_database_description__hnsw
    on ix.pg_idx_emb_e5_1024 using hnsw (emb halfvec_cosine_ops)
