...
переносе на Python.
*/
-- @requires ix.pg_llm_description
-- @name queue
-- @params batch
with col as (
...
aspect as (
    select o.node_id, o.surface, 'meta_description'::ix.pg_idx_aspect_e as aspect,
    ...
    union all
    select s.node_id, s.surface, 'llm_description', s.content::varchar
    from ix.pg_llm_description s
),
todo as (
    select a.node_id, a.surface, a.aspect, a.content, md5(a.content) as content_hash
    from aspect a
    left join (select distinct node_id, surface, aspect, content_hash from ix.pg_idx_emb_e5_1024) e
