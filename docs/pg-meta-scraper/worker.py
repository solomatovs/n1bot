...
class Pipeline:
    ...
    def run(self) -> Sequence[ApplyRow]:
        with psycopg.connect(self._cfg.source_dsn, autocommit=True, application_name="pg-meta-scraper") as src, \
             psycopg.connect(self._cfg.ix_dsn, autocommit=True, application_name="pg-meta-scraper") as ix:
...
class Cli:
    ...
    def parse(cls, argv: Sequence[str] | None = None) -> WorkerConfig:
        parser = argparse.ArgumentParser(description="pg-meta-scraper worker")
