/*
pg-idx-trgm, шаг 1: вставить недостающие строки и обновить те, у которых content
...
with col as (
    ...
    from ix.pg_meta_column c
    join ix.tree t on t.node_id = c.node_id
    group by t.parent_id
),
obj as (
    select x.node_id, 'pg_meta_database'::ix.surface_e as surface, x.name, null::varchar as schema_name, x.name as path,
           'Database ' || x.name as head, x.comment, null::varchar as columns, null::varchar as typed
    from ix.pg_meta_database x
    union all
    select x.node_id, 'pg_meta_schema', x.name, x.name, x.name, 'Schema ' || x.name, x.comment, null, null
    from ix.pg_meta_schema x
    union all
    select x.node_id, 'pg_meta_table', x.name, x.schema_name, x.schema_name || '.' || x.name,
    ...
    from ix.pg_meta_table x left join col on col.rel_id = x.node_id
    union all
    select x.node_id, 'pg_meta_column', x.name, x.schema_name, x.schema_name || '.' || x.relation_name || '.' || x.name,
           'Column ' || x.schema_name || '.' || x.relation_name || '.' || x.name || ' ' || x.data_type, x.comment, null, null
    from ix.pg_meta_column x
    union all
    select x.node_id, 'pg_meta_view', x.name, x.schema_name, x.schema_name || '.' || x.name,
           case x.kind when 'matview' then 'Materialized view ' else 'View ' end || x.schema_name || '.' || x.name,
           x.comment, col.names, col.typed
    from ix.pg_meta_view x left join col on col.rel_id = x.node_id
    union all
    select x.node_id, 'pg_meta_index', x.name, x.schema_name, x.schema_name || '.' || x.name,
           'Index ' || x.name || ' on ' || x.table_name || ' (' || array_to_string(x.columns, ', ') || ')' || case when x.is_unique then ' unique' else '' end,
           x.comment, null, null
    from ix.pg_meta_index x
    union all
    select x.node_id, 'pg_meta_sequence', x.name, x.schema_name, x.schema_name || '.' || x.name, 'Sequence ' || x.schema_name || '.' || x.name, x.comment, null, null
    from ix.pg_meta_sequence x
    union all
    select x.node_id, 'pg_meta_routine', x.name, x.schema_name, x.schema_name || '.' || x.name || '(' || coalesce(x.identity_args, '') || ')',
           initcap(x.kind) || ' ' || x.name || '(' || coalesce(x.identity_args, '') || ')' || coalesce(' returns ' || x.result_type, ''), x.comment, null, null
    from ix.pg_meta_routine x
    union all
    select x.node_id, 'pg_meta_constraint', x.name, x.schema_name, x.schema_name || '.' || x.table_name || '.' || x.name,
           initcap(x.kind) || ' ' || x.name || ' on ' || coalesce(x.table_name, '') || ': ' || x.definition, x.comment, null, null
    from ix.pg_meta_constraint x
    union all
    select x.node_id, 'pg_meta_trigger', x.name, x.schema_name, x.schema_name || '.' || x.table_name || '.' || x.name,
           'Trigger ' || x.name || ' on ' || x.table_name || ' ' || x.timing || ' ' || array_to_string(x.events, ', '), x.comment, null, null
    from ix.pg_meta_trigger x
    union all
    select x.node_id, 'pg_meta_type', x.name, x.schema_name, x.schema_name || '.' || x.name,
           initcap(x.kind) || ' ' || x.schema_name || '.' || x.name || coalesce(' ' || x.base_type, '') || coalesce(' (' || array_to_string(x.enum_labels, ', ') || ')', ''), x.comment, null, null
    from ix.pg_meta_type x
    union all
    select x.node_id, 'pg_meta_statistics', x.name, x.schema_name, x.schema_name || '.' || x.name, 'Statistics ' || x.name || ' on ' || x.table_name, x.comment, null, null
    from ix.pg_meta_statistics x
),
aspect as (
    select o.node_id, o.surface, 'meta_name'::ix.pg_idx_aspect_e as aspect, o.name::varchar as content from obj o
    union all
