...
```
.venv/bin/python docs/pg-idx-trgm/worker.py --dsn "host=<хост ix> port=5432 dbname=<база ix> user=<роль> password=<пароль>"
```
...
```
.venv/bin/python docs/pg-idx-trgm/worker.py --dsn "host=... dbname=... user=... password=..." [--batch 500]
```
