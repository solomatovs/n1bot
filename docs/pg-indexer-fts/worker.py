...
logger = logging.getLogger("pg-indexer-fts")
    ...
    def run(self) -> CycleReport:
        try:
            with psycopg.connect(
                self._cfg.dsn, autocommit=True, application_name="pg-indexer-fts"
            ) as conn:
                conn.execute(
...
class Cli:
    ...
    def parse(cls, argv: Sequence[str] | None = None) -> WorkerConfig:
        parser = argparse.ArgumentParser(description="pg-indexer-fts worker")
