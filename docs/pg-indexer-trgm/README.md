# pg-indexer-trgm: триграммный и btree-индекс имён объектов PostgreSQL

Независимый индексатор поверх схемы `ix`. Читает только `ix.node`, `ix.tree` и surface-таблицы
`ix.pg_*`, пишет только в `ix.pg_trgm`. Со скрапером не связан ни вызовами, ни внешними
...
`schema/00_pg_trgm.sql`: словарь аспектов `ix.pg_aspect_e` и `ix.pg_aspect` (общий для
индексаторов, каждый создаёт его идемпотентно) и таблица `ix.pg_trgm` с индексами.
