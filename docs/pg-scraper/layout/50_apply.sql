...
select count(*) as locked_pg_edge from (
    select m.edge_id, m.role, m.side, m.ordinal from ix.pg_edge m
    join ix.edge e on e.id = m.edge_id
    where exists (select 1 from scope_node sc where sc.id = e.node_src_id or sc.id = e.node_tgt_id)
    order by m.edge_id, m.role, m.side, m.ordinal for update of m
) l;

select count(*) as locked_pg_database from (
    select x.node_id from ix.pg_database x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_schema from (
    select x.node_id from ix.pg_schema x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_table from (
    select x.node_id from ix.pg_table x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_column from (
    select x.node_id from ix.pg_column x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_view from (
    select x.node_id from ix.pg_view x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_index from (
    select x.node_id from ix.pg_index x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_sequence from (
    select x.node_id from ix.pg_sequence x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_routine from (
    select x.node_id from ix.pg_routine x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_constraint from (
    select x.node_id from ix.pg_constraint x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_trigger from (
    select x.node_id from ix.pg_trigger x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_type from (
    select x.node_id from ix.pg_type x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;

select count(*) as locked_pg_statistics from (
    select x.node_id from ix.pg_statistics x join scope_node sc on sc.id = x.node_id order by x.node_id for update of x
) l;
...
with cur as (
    ...
    from ix.pg_edge m join ix.edge e on e.id = m.edge_id join scope_node sc on sc.id = e.node_src_id
), want as (
    ...
    delete from ix.pg_edge x using del d
    where x.edge_id = d.edge_id and x.role = d.role::ix.pg_edge_role_e and x.side = d.side and x.ordinal = d.ordinal
    returning x.edge_id
)
insert into apply_log select 'pg_edge_delete', (select count(*) from del), (select count(*) from done);
...
with cur as (
    ...
    insert into ix.pg_edge (edge_id, role, side, ordinal, is_key)
    select edge_id, role::ix.pg_edge_role_e, side, ordinal, is_key from ins
    on conflict (edge_id, role, side, ordinal) do nothing
    returning edge_id
)
insert into apply_log select 'pg_edge_insert', (select count(*) from ins), (select count(*) from done);

-- 6. surface node: строка целиком против строки целиком, ключ node_id

with cur as (
    select x.node_id, name, owner, encoding, collate_name, ctype, comment from ix.pg_database x join scope_node sc on sc.id = x.node_id
), want as (
    select nm.id as node_id, st.name, st.owner, st.encoding, st.collate_name, st.ctype, st.comment from stage_pg_database st join node_map nm on nm.address = st.address
), del as (
    ...
    delete from ix.pg_database x using del d where x.node_id = d.node_id returning x.node_id
)
insert into apply_log select 'pg_database_delete', (select count(*) from del), (select count(*) from done);
...
with cur as (
    ...
    insert into ix.pg_database (node_id, name, owner, encoding, collate_name, ctype, comment)
    select node_id, name, owner, encoding, collate_name, ctype, comment from ins
    on conflict (node_id) do nothing
    returning node_id
)
insert into apply_log select 'pg_database_insert', (select count(*) from ins), (select count(*) from done);

with cur as (
    select x.node_id, name, owner, comment from ix.pg_schema x join scope_node sc on sc.id = x.node_id
), want as (
    select nm.id as node_id, st.name, st.owner, st.comment from stage_pg_schema st join node_map nm on nm.address = st.address
), del as (
    select c.node_id from cur c
    ...
    delete from ix.pg_schema x using del d where x.node_id = d.node_id returning x.node_id
)
insert into apply_log select 'pg_schema_delete', (select count(*) from del), (select count(*) from done);
...
with cur as (
    ...
    insert into ix.pg_schema (node_id, name, owner, comment)
    select node_id, name, owner, comment from ins
    on conflict (node_id) do nothing
    returning node_id
)
insert into apply_log select 'pg_schema_insert', (select count(*) from ins), (select count(*) from done);

with cur as (
    select x.node_id, schema_name, name, kind, owner, tablespace, persistence, partition_bound, row_estimate, pages, has_index, has_triggers, distribution, storage, comment from ix.pg_table x join scope_node sc on sc.id = x.node_id
), want as (
    select nm.id as node_id, st.schema_name, st.name, st.kind, st.owner, st.tablespace, st.persistence, st.partition_bound, st.row_estimate, st.pages, st.has_index, st.has_triggers, st.distribution, st.storage, st.comment from stage_pg_table st join node_map nm on nm.address = st.address
), del as (
    select c.node_id from cur c
    ...
    delete from ix.pg_table x using del d where x.node_id = d.node_id returning x.node_id
)
insert into apply_log select 'pg_table_delete', (select count(*) from del), (select count(*) from done);
...
with cur as (
    ...
    insert into ix.pg_table (node_id, schema_name, name, kind, owner, tablespace, persistence, partition_bound, row_estimate, pages, has_index, has_triggers, distribution, storage, comment)
...
insert into apply_log select 'pg_table_insert', (select count(*) from ins), (select count(*) from done);

with cur as (
    select x.node_id, schema_name, relation_name, relation_kind, name, ordinal, data_type, not_null, default_expr, identity, generated, comment from ix.pg_column x join scope_node sc on sc.id = x.node_id
), want as (
    select nm.id as node_id, st.schema_name, st.relation_name, st.relation_kind, st.name, st.ordinal, st.data_type, st.not_null, st.default_expr, st.identity, st.generated, st.comment from stage_pg_column st join node_map nm on nm.address = st.address
), del as (
    select c.node_id from cur c
    ...
    delete from ix.pg_column x using del d where x.node_id = d.node_id returning x.node_id
)
insert into apply_log select 'pg_column_delete', (select count(*) from del), (select count(*) from done);
...
with cur as (
    ...
    insert into ix.pg_column (node_id, schema_name, relation_name, relation_kind, name, ordinal, data_type, not_null, default_expr, identity, generated, comment)
...
insert into apply_log select 'pg_column_insert', (select count(*) from ins), (select count(*) from done);

with cur as (
    select x.node_id, schema_name, name, kind, owner, comment from ix.pg_view x join scope_node sc on sc.id = x.node_id
), want as (
    select nm.id as node_id, st.schema_name, st.name, st.kind, st.owner, st.comment from stage_pg_view st join node_map nm on nm.address = st.address
), del as (
    select c.node_id from cur c
    ...
    delete from ix.pg_view x using del d where x.node_id = d.node_id returning x.node_id
)
insert into apply_log select 'pg_view_delete', (select count(*) from del), (select count(*) from done);
...
with cur as (
    ...
    insert into ix.pg_view (node_id, schema_name, name, kind, owner, comment)
    select node_id, schema_name, name, kind, owner, comment from ins
    on conflict (node_id) do nothing
    returning node_id
)
insert into apply_log select 'pg_view_insert', (select count(*) from ins), (select count(*) from done);

with cur as (
    select x.node_id, schema_name, table_name, name, access_method, is_unique, is_primary, is_exclusion, is_valid, columns, expression, predicate, comment from ix.pg_index x join scope_node sc on sc.id = x.node_id
), want as (
    select nm.id as node_id, st.schema_name, st.table_name, st.name, st.access_method, st.is_unique, st.is_primary, st.is_exclusion, st.is_valid, st.columns, st.expression, st.predicate, st.comment from stage_pg_index st join node_map nm on nm.address = st.address
), del as (
    select c.node_id from cur c
    ...
    delete from ix.pg_index x using del d where x.node_id = d.node_id returning x.node_id
)
insert into apply_log select 'pg_index_delete', (select count(*) from del), (select count(*) from done);
...
with cur as (
    ...
    insert into ix.pg_index (node_id, schema_name, table_name, name, access_method, is_unique, is_primary, is_exclusion, is_valid, columns, expression, predicate, comment)
...
insert into apply_log select 'pg_index_insert', (select count(*) from ins), (select count(*) from done);

with cur as (
    select x.node_id, schema_name, name, owner, data_type, start_value, increment, min_value, max_value, cycle, comment from ix.pg_sequence x join scope_node sc on sc.id = x.node_id
), want as (
    select nm.id as node_id, st.schema_name, st.name, st.owner, st.data_type, st.start_value, st.increment, st.min_value, st.max_value, st.cycle, st.comment from stage_pg_sequence st join node_map nm on nm.address = st.address
), del as (
    select c.node_id from cur c
    ...
    delete from ix.pg_sequence x using del d where x.node_id = d.node_id returning x.node_id
)
insert into apply_log select 'pg_sequence_delete', (select count(*) from del), (select count(*) from done);
...
with cur as (
    ...
    insert into ix.pg_sequence (node_id, schema_name, name, owner, data_type, start_value, increment, min_value, max_value, cycle, comment)
    select node_id, schema_name, name, owner, data_type, start_value, increment, min_value, max_value, cycle, comment from ins
    on conflict (node_id) do nothing
    returning node_id
)
insert into apply_log select 'pg_sequence_insert', (select count(*) from ins), (select count(*) from done);

with cur as (
    select x.node_id, schema_name, name, kind, language, identity_args, result_type, volatility, security_definer, owner, comment from ix.pg_routine x join scope_node sc on sc.id = x.node_id
), want as (
    select nm.id as node_id, st.schema_name, st.name, st.kind, st.language, st.identity_args, st.result_type, st.volatility, st.security_definer, st.owner, st.comment from stage_pg_routine st join node_map nm on nm.address = st.address
), del as (
    select c.node_id from cur c
    ...
    delete from ix.pg_routine x using del d where x.node_id = d.node_id returning x.node_id
)
insert into apply_log select 'pg_routine_delete', (select count(*) from del), (select count(*) from done);
...
with cur as (
    ...
    insert into ix.pg_routine (node_id, schema_name, name, kind, language, identity_args, result_type, volatility, security_definer, owner, comment)
...
insert into apply_log select 'pg_routine_insert', (select count(*) from ins), (select count(*) from done);

with cur as (
    select x.node_id, schema_name, table_name, name, kind, definition, is_deferrable, is_deferred, is_validated, on_update, on_delete, match_type, comment from ix.pg_constraint x join scope_node sc on sc.id = x.node_id
), want as (
    select nm.id as node_id, st.schema_name, st.table_name, st.name, st.kind, st.definition, st.is_deferrable, st.is_deferred, st.is_validated, st.on_update, st.on_delete, st.match_type, st.comment from stage_pg_constraint st join node_map nm on nm.address = st.address
), del as (
    select c.node_id from cur c
    ...
    delete from ix.pg_constraint x using del d where x.node_id = d.node_id returning x.node_id
)
insert into apply_log select 'pg_constraint_delete', (select count(*) from del), (select count(*) from done);
...
with cur as (
    ...
    insert into ix.pg_constraint (node_id, schema_name, table_name, name, kind, definition, is_deferrable, is_deferred, is_validated, on_update, on_delete, match_type, comment)
...
insert into apply_log select 'pg_constraint_insert', (select count(*) from ins), (select count(*) from done);

with cur as (
    select x.node_id, schema_name, table_name, name, timing, events, row_level, enabled, comment from ix.pg_trigger x join scope_node sc on sc.id = x.node_id
), want as (
    select nm.id as node_id, st.schema_name, st.table_name, st.name, st.timing, st.events, st.row_level, st.enabled, st.comment from stage_pg_trigger st join node_map nm on nm.address = st.address
), del as (
    select c.node_id from cur c
    ...
    delete from ix.pg_trigger x using del d where x.node_id = d.node_id returning x.node_id
)
insert into apply_log select 'pg_trigger_delete', (select count(*) from del), (select count(*) from done);
...
with cur as (
    ...
    insert into ix.pg_trigger (node_id, schema_name, table_name, name, timing, events, row_level, enabled, comment)
    select node_id, schema_name, table_name, name, timing, events, row_level, enabled, comment from ins
    on conflict (node_id) do nothing
    returning node_id
)
insert into apply_log select 'pg_trigger_insert', (select count(*) from ins), (select count(*) from done);

with cur as (
    select x.node_id, schema_name, name, kind, base_type, enum_labels, comment from ix.pg_type x join scope_node sc on sc.id = x.node_id
), want as (
    select nm.id as node_id, st.schema_name, st.name, st.kind, st.base_type, st.enum_labels, st.comment from stage_pg_type st join node_map nm on nm.address = st.address
), del as (
    select c.node_id from cur c
    ...
    delete from ix.pg_type x using del d where x.node_id = d.node_id returning x.node_id
)
insert into apply_log select 'pg_type_delete', (select count(*) from del), (select count(*) from done);
...
with cur as (
    ...
    insert into ix.pg_type (node_id, schema_name, name, kind, base_type, enum_labels, comment)
    select node_id, schema_name, name, kind, base_type, enum_labels, comment from ins
    on conflict (node_id) do nothing
    returning node_id
)
insert into apply_log select 'pg_type_insert', (select count(*) from ins), (select count(*) from done);

with cur as (
    select x.node_id, schema_name, name, table_name, kinds, comment from ix.pg_statistics x join scope_node sc on sc.id = x.node_id
), want as (
    select nm.id as node_id, st.schema_name, st.name, st.table_name, st.kinds, st.comment from stage_pg_statistics st join node_map nm on nm.address = st.address
), del as (
    select c.node_id from cur c
    ...
    delete from ix.pg_statistics x using del d where x.node_id = d.node_id returning x.node_id
)
insert into apply_log select 'pg_statistics_delete', (select count(*) from del), (select count(*) from done);
...
with cur as (
    ...
    insert into ix.pg_statistics (node_id, schema_name, name, table_name, kinds, comment)
    select node_id, schema_name, name, table_name, kinds, comment from ins
    on conflict (node_id) do nothing
    returning node_id
)
insert into apply_log select 'pg_statistics_insert', (select count(*) from ins), (select count(*) from done);
