...
инкрементируемое :class:`~boba_2.domain.agent.meat.loop_control.\
...
class AgentConfig:
    ...
    :class:`~boba_2.domain.agent.meat.tools.\
RepeatedToolCallGuardMiddleware` — N+1-й вызов подавляется как
    :class:`~boba_2.domain.agent.errors.ToolFeedbackError`.
...
RepeatedFormatFailureGuardMiddleware` — после N+1 эмитится
    :class:`~boba_2.domain.agent.events.RepeatedFormatFailure` и цикл
    останавливается.
    """
...
class AgentContext:
    ...
    каждой итерации :class:`~boba_2.domain.agent.meat.loop_control.\
