...
class TerminalFailure(BaseAgentEvent, ABC):
    ...
    :class:`~boba_2.domain.agent.meat.loop_control.StopOnAnyFailure`.
    """
...
class UserQueryReceived(DurableMessage):
    ...
    Эмитится один раз — :class:`~boba_2.domain.agent.meat.prompt.\
...
class ToolCallComplete(DurableMessage):
    ...
    Эмитится :class:`~boba_2.domain.agent.meat.dialogue.\
AssistantMessagePersistenceMiddleware` при ``GenerationDone``
    — после накопления ``ToolCallBegin`` + серии
    ``ToolCallArgumentDelta``. :class:`~boba_2.domain.agent.meat.\
...
class ToolExecutionStarted(LifecycleMarker):
    ...
    Эмитится :class:`~boba_2.domain.agent.meat.tools.\
...
class ToolCallArgumentDelta(StreamingDelta):
    ...
    :class:`ToolCallBegin`. :class:`~boba_2.domain.agent.meat.\
